...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
  	var piza=document.getElementByClassName('randomPizzaContainer');
  	var dx = determineDx(piza[0], size);
    var newwidth = (piza[0].offsetWidth + dx) + 'px';
    for (var i = 0; i < document.querySelectorAll(".randomPizzaContainer").length; i++) {   
      piza[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  var t=(document.body.scrollTop / 1250);
  for (var i = 0; i < items.length; i++) {
    phase = Math.sin(t + (i % 5));
